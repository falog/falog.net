...
* ADD: 指定された2つのレジスタの値を加算し、
    結果を最初のレジスタに格納する。
![ADD](/src/assets/003_cpu-minimal-implementation/add.png)
* SUB: 指定された2つのレジスタの値を減算し、
    結果を最初のレジスタに格納する。
![SUB](/src/assets/003_cpu-minimal-implementation/sub.png)
* LD: 指定されたメモリアドレスから値を読み取り、
    指定されたレジスタに格納する。
![LD](/src/assets/003_cpu-minimal-implementation/ld.png)
* ST: 指定されたレジスタの値を読み取り、
    指定されたメモリアドレスに書き込む。
![ST](/src/assets/003_cpu-minimal-implementation/st.png)
* JMP: 指定されたアドレスに無条件でジャンプする。   
    ジャンプ先は命令インデックスで指定され、内部で3倍される。
![JMP](/src/assets/003_cpu-minimal-implementation/jmp.png)
* JE: 指定されたレジスタの値が0の場合に、
    指定されたアドレスにジャンプする。そうでない場合は次の
    命令に進む。
![JE](/src/assets/003_cpu-minimal-implementation/je.png)
* MOV: 指定された2つのレジスタの値をコピーする。
![MOV](/src/assets/003_cpu-minimal-implementation/mov.png)
* HLT: CPUの実行を停止する。
![HLT](/src/assets/003_cpu-minimal-implementation/hlt.png)
